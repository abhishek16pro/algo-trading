...
export const authRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/login', async (req) => {
    ...
    // TODO: TOTP check when user.twoFactor.enabled
    return await app.issueTokens({ sub: String(user._id), email: user.email });
  });
  ...
  app.post('/logout', { preHandler: app.authenticate }, async (req) => {
    // Caller can include jti from refresh token to revoke specifically; otherwise revoke all.
